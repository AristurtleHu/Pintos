...
[link](https://courses.cs.vt.edu/~cs3204/spring2009/pintos-vt-local/Project3Session.pdf) to the tutorial ppt 
...
In my implementation (primarily in `frame.c`), races are avoided when two user processes concurrently need a new frame by using a single global mutex called `frame_lock`. 

The `frame_alloc()` function, which is responsible for providing a physical frame, acquires `frame_lock` at its beginning and holds it until the entire allocation process, including any potential page eviction via `evict_frame()`, is complete. 
This lock protects all critical operations involving shared frame management data structures, such as checking the `palloc` system for free pages, selecting a victim frame using the clock algorithm within `evict_frame()`, modifying the global `frame_table` list (e.g., adding a new frame entry or removing an evicted one), and interacting with the swap system via `swap_out()`. 

By serializing these operations, `frame_lock` ensures that the frame allocation and eviction logic behaves as an atomic unit with respect to concurrent requests, thus preventing race conditions.

### Rationale

> **A5:** Why did you choose the data structure(s) that you did for representing virtual-to-physical mappings?

For representing virtual-to-physical (or to backing store) mappings at the operating system level, I chose a per-thread hash table to implement the Supplemental Page Table, where each entry is a `struct sup_page_table_entry`. 
I selected a hash table primarily because it offers efficient average O(1) time complexity for looking up an SPTE using the page-aligned user virtual address (`uaddr`) as the key; this is critical for fast page fault handling, overlap checks, and managing potentially sparse user virtual address spaces. 
The `struct sup_page_table_entry` itself is designed to store all necessary metadata to manage a virtual page throughout its lifecycle, including its `type` (e.g., `PAGE_FILE`, `PAGE_ZERO`, `PAGE_STACK`), backing store information (like a `file` pointer, `offset` in the file, `read_bytes`, and `zero_bytes` for file-backed pages), its current status and location (e.g., kernel virtual address `kaddr` if in a frame, or a `swap_index` if swapped out), and its access permissions (`writable`).
...
My frame eviction strategy, implemented in the `evict_frame()` function within `frame.c`, uses a second-chance algorithm. 

I maintain a global `frame_table` (a list of `frame_table_entry` structs) and a `clock_ptr` that circularly traverses this list. When eviction is necessary, my code iterates from the `clock_ptr`'s current position: 
1. if a frame is pinned (`fte->pinned`), it is skipped. For an unpinned frame, I check its accessed bit using `pagedir_is_accessed()`; 
2. if this bit is set, I clear it using `pagedir_set_accessed(..., false)` to give the page a second chance, and the clock hand advances. 
3. If the accessed bit is already clear, that frame is chosen as the victim. Its content is then written to a swap slot via `swap_out()`, its hardware page table entry is cleared using `pagedir_clear_page()`, its Supplemental Page Table Entry is updated (setting `kaddr = NULL` and storing the `swap_index`), the `frame_table_entry` struct is freed, and the kernel virtual address of the now-available physical frame is returned for reuse.
...
My heuristic for deciding whether a page fault for an invalid virtual address should extend the stack combines address range and spatial proximity checks:

1. Address Range Validation: I verify that `fault_addr` is within legal stack boundaries by checking that it's above `PHYS_BASE - MAX_STACK_SIZE` (where `MAX_STACK_SIZE` is typically 8MB), ensuring the address falls within the maximum allowed stack size.

2. ESP Proximity Check: I ensure `fault_addr` is not too far from the current stack pointer by verifying that `fault_addr >= esp - 32`. This 32-byte is given by the pdf and prevents arbitrary memory accesses from being treated as legitimate stack growth.

When both conditions are satisfied, I call `stack_grow(fault_addr, !user)` to allocate a new page frame, zero-fill it, update the hardware page table, and create a proper supplemental page table entry of type `FRAME` for the new stack page. 

The `!user` parameter indicates whether to keep the frame pinned (typically only done for kernel operations). If either check fails, the access is considered invalid and triggers process termination via `exit(-1)`.
...
My VM synchronization design falls in the middle of the lock granularity spectrum, balancing parallelism with implementation simplicity. I use several targeted locks rather than a single global VM lock:

1. A global `frame_lock` that protects the frame table and frame allocation/eviction operations
2. Per-SPTE locks (`spte_lock`) that protect individual page state transitions
3. A global `swap_lock` for swap slot allocation and disk operations
4. A global `filesys_lock` for file system operations

This approach allows some parallelism - multiple processes can simultaneously fault on different pages that don't require eviction, and file operations can proceed independently of swap operations. However, it avoids the extreme complexity of fine-grained locking (e.g., per-frame locks).

I chose this design because it provides reasonable concurrency while keeping synchronization logic manageable. Frame allocation/eviction is a central bottleneck, so serializing it with one lock makes sense. The per-SPTE locks allow concurrent page fault handling when frames are available. The separate locks for swap and filesystem operations prevent unnecessary blocking between these subsystems.
